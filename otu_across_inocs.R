...
trends<-function(Phylolevel,Inocs,lim){
  ...
  #=========Remove OTUs that are never above lim in inocs===========================================================================================
  q<-1  
  good <- c()    # to be used in the following if statements
  for(h in 6:ncol(meta)){  # start at first OTU column
    
    test<-which(meta[,h]>=lim)
    good_Inocs<-unique(meta$Inoc[test])  # which inocula have at least 1 sample from this OTU above the limit
    if(length(good_Inocs)>=length(Inocs)){  # should never be greater than but we just want to get rid of things that are less than
  ...
  print(paste("There are",length(good), "matching OTUs",sep=" "))
  ...
  for(c in 6:ncol(meta)){  # highlight 1 OTU at a time there are 5 columns of meta data
    ...
    #+++++++++++++++set up Plotting+++++++++++++++++++++++++++++++++++++++++++
    plot(1, type='n', xlim=c(-1,30), ylim=c(0,1), ylab="Relative Abundance", xlab="Day", main = paste(get_name(names(meta[c])),'(',names(meta[c]),')',"at level:",Phylolevel))
    ...
    legend(23,1,key,pch=16,col=key_colors) # takes color from distriubution of j which corresponds to cages
